...
test("tailored fields follow each job focus and never add fixed role-specific sections", () => {
  const value = fixture();
  let firstPackage;
  try {
    ...
    assert.equal(firstPackage.content.sections.some((section) => ["motivation", "plan", "role_specific_topic", "viewpoint"].includes(section.key)), false);
...
test("changing a protected career stage makes an existing package stale", () => {
  const value = fixture();
  let packageValue;
  try {
    packageValue = createPackage(value.db, value.jobId);
    value.db.prepare("UPDATE resume_profile SET career_stage = 'returning', updated_at = CURRENT_TIMESTAMP WHERE id = 1").run();
    const stale = getLatestPackageForJob(value.db, value.jobId);
    assert.equal(stale.refreshRequired, true);
    assert.deepEqual(stale.refreshReasons.map((reason) => reason.key), ["base_resume_changed"]);
  } finally {
    value.cleanup(packageValue);
