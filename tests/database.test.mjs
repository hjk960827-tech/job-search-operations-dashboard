...
test("custom resume sections cannot duplicate built-in section semantics", () => {
  const temp = temporaryDatabase();
  try {
    initializeDatabase(temp.file, { mode: "personal" });
    const db = openDatabase(temp.file);
    const beforeProfile = db.prepare("SELECT * FROM resume_profile ORDER BY id").all();
    const beforeSections = db.prepare("SELECT * FROM resume_custom_sections ORDER BY id").all();
    const duplicateVariants = [
      { id: "duplicate-key", key: "custom:summary", label: "별도 요약", kind: "text", value: "중복" },
      { id: "duplicate-label", key: "custom:anything", label: "경력 요약", kind: "text", value: "중복" },
    ];
    for (const customSection of duplicateVariants) {
      assertClientError(() => saveResume(db, {
        careerStage: "experienced",
        headline: "검증 전에는 저장되면 안 됩니다.",
        customSections: [customSection],
      }));
      assert.deepEqual(db.prepare("SELECT * FROM resume_profile ORDER BY id").all(), beforeProfile);
      assert.deepEqual(db.prepare("SELECT * FROM resume_custom_sections ORDER BY id").all(), beforeSections);
    }

    saveResume(db, {
      careerStage: "experienced",
      customSections: [{ id: "projects", key: "custom:projects", label: "프로젝트", kind: "text", value: "검증된 프로젝트" }],
    });
    assert.deepEqual(
      db.prepare("SELECT section_key FROM resume_custom_sections").all().map((item) => item.section_key),
      ["custom:projects"],
    );
    db.close();
  } finally {
