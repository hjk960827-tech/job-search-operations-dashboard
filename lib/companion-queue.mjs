...
function packageGenerationInput(db, request) {
  ...
  const structuredItems = db.prepare(`
    SELECT id, kind, title, organization, role, engagement_type, location, start_date, end_date, summary,
           highlights_json, skills_json, portfolio_links_json
  ...
  `).all().map((item) => ({
    ...
    role: item.role, engagementType: item.engagement_type, location: item.location, startDate: item.start_date, endDate: item.end_date,
    summary: item.summary, highlights: parseJson(item.highlights_json, []), skills: parseJson(item.skills_json, []),
    portfolioLinks: parseJson(item.portfolio_links_json, []),
  }));
