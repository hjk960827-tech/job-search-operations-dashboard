...
import { normalizeDocumentQualityCriteria } from "./document-quality.mjs";
...
const QUALITY_ENGINE_VERSION = "package-quality-v2";
...
function qualityRulesSnapshot(options = {}, fallback = null) {
  const hasCurrentRules = Object.hasOwn(options, "threshold")
    ...
    || Object.hasOwn(options, "qualityCriteria");
  ...
  return stableValue({
    ...
    criteria: normalizeDocumentQualityCriteria(options.qualityCriteria) || undefined,
...
function resumeFingerprintInput(resume = {}) {
  ...
  const columns = new Set([
    "job_family", "job_role", "career_type", "career_stage", "years_experience", "school", "major",
  ...
  result.custom_sections = stableValue(Array.isArray(resume.custom_sections) ? resume.custom_sections : []);
  result.confirmed_facts = stableValue(Array.isArray(resume.confirmed_facts) ? resume.confirmed_facts : []);
  result.structured_items = stableValue(Array.isArray(resume.structured_items) ? resume.structured_items : []);
  result.contacts = stableValue(Array.isArray(resume.contacts) ? resume.contacts : []);
  return result;
}
...
function normalizeContactItems(value) {
  const allowed = new Set(["email", "phone", "address"]);
  const seen = new Set();
  return (Array.isArray(value) ? value : []).slice(0, 3).map((item) => {
    ...
    const contactValue = cleanText(item?.value, 500);
    return label && contactValue ? { key, label, value: contactValue } : null;
  }).filter(Boolean);
}

function normalizeStructuredPackageItems(value) {
  const allowed = new Set(["experience", "education", "skill", "certification", "project"]);
  const seen = new Set();
  return (Array.isArray(value) ? value : []).slice(0, 200).map((item) => {
    const id = cleanText(item?.id, 100);
    const kind = cleanText(item?.kind, 40);
    if (!id || !allowed.has(kind) || seen.has(id) || item?.active === false) return null;
    seen.add(id);
    ...
    return {
      ...
      engagementType: cleanText(item?.engagementType, 200),
      location: cleanText(item?.location, 200),
      startDate: cleanText(item?.startDate, 10),
      endDate: cleanText(item?.endDate, 10),
      summary: cleanText(item?.summary, 8000),
      highlights: cleanLines(item?.highlights, 50),
      skills: cleanLines(item?.skills, 50),
      portfolioLinks: cleanLines(item?.portfolioLinks, 30),
    };
  }).filter(Boolean);
}
...
export function normalizePackageContent(input = {}) {
  ...
  const sections = sourceSections.slice(0, 50).map((section) => {
    ...
    return {
      ...
      sourceRefs: cleanLines(section?.sourceRefs, 50),
    };
  }).filter(Boolean);
  ...
  return {
    sections,
    protectedFacts,
    contacts: normalizeContactItems(input.contacts),
    structuredItems: normalizeStructuredPackageItems(input.structuredItems),
    selection: {
...
export function renderPackageMarkdown(content, job = {}) {
  ...
  if (normalized.contacts.length) {
    lines.push(normalized.contacts.map((item) => `${item.label}: ${item.value}`).join(" · "), "");
  }
  ...
  const structuredLabels = { experience: "경력", education: "학력", skill: "기술", certification: "자격·인증", project: "프로젝트" };
  for (const kind of Object.keys(structuredLabels)) {
    const items = normalized.structuredItems.filter((item) => item.kind === kind);
    if (!items.length) continue;
    lines.push(`## ${structuredLabels[kind]}`, "");
    for (const item of items) {
      const metadata = [item.organization, item.role, item.engagementType, [item.startDate, item.endDate].filter(Boolean).join(" ~ "), item.location].filter(Boolean).join(" · ");
      lines.push(`### ${item.title}`, metadata, item.summary, ...item.highlights.map((entry) => `- ${entry}`));
      if (item.skills.length) lines.push(`- 기술: ${item.skills.join(", ")}`);
      if (item.portfolioLinks.length) lines.push(`- 연결 자료: ${item.portfolioLinks.join(" · ")}`);
      lines.push("");
    }
  }
  return lines.join("\n");
...
export function renderPackageHtml(content, job = {}) {
  ...
  const facts = normalized.protectedFacts.length
    ...
    : "";
  const contacts = normalized.contacts.length
    ? `<div class="contacts">${normalized.contacts.map((item) => `<span><strong>${escapeHtml(item.label)}</strong> ${escapeHtml(item.value)}</span>`).join("")}</div>`
    : "";
  ...
  const structuredLabels = { experience: "경력", education: "학력", skill: "기술", certification: "자격·인증", project: "프로젝트" };
  const structured = Object.entries(structuredLabels).map(([kind, label]) => {
    const items = normalized.structuredItems.filter((item) => item.kind === kind);
    if (!items.length) return "";
    return `<section><h2>${escapeHtml(label)}</h2>${items.map((item) => {
      const metadata = [item.organization, item.role, item.engagementType, [item.startDate, item.endDate].filter(Boolean).join(" ~ "), item.location].filter(Boolean).join(" · ");
      return `<article class="structured-item"><h3>${escapeHtml(item.title)}</h3>${metadata ? `<div class="meta">${escapeHtml(metadata)}</div>` : ""}${prose(item.summary)}${item.highlights.length ? `<ul>${list(item.highlights)}</ul>` : ""}${item.skills.length ? `<div class="skills">${escapeHtml(item.skills.join(" · "))}</div>` : ""}${item.portfolioLinks.length ? `<div class="skills">연결 자료 · ${escapeHtml(item.portfolioLinks.join(" · "))}</div>` : ""}</article>`;
    }).join("")}</section>`;
  }).join("");
  return `<!doctype html>
...
.contacts { display: flex; flex-wrap: wrap; gap: 1.5mm 5mm; margin: -1mm 0 4mm; color: #405149; font-size: 9.5pt; } .contacts strong { margin-right: 1mm; }
...
.structured-item { break-inside: avoid; margin: 0 0 4mm; } .structured-item h3 { margin: 0 0 1mm; font-size: 10.8pt; } .meta, .skills { color: #52635c; font-size: 9.5pt; margin-bottom: 1.5mm; }
</style></head><body>
<h1>${escapeHtml(headline)}</h1>
${contacts}
<div class="target">${escapeHtml(job.companyName || "")} · ${escapeHtml(job.title || "")}</div>
${facts}${sections}${structured}
</body></html>`;
}

export function evaluatePackageQuality(content, { threshold = 80, criteria = null } = {}) {
  const normalized = normalizePackageContent(content);
  ...
  const finalOutputValues = {
    ...
    structuredItems: normalized.structuredItems,
  };
  ...
  const score = resumeBodySections.length
    ...
    : 0;
  const normalizedCriteria = normalizeDocumentQualityCriteria(criteria);
  const focusKeys = new Set(normalized.selection.focusSections || []);
  const presentKeys = new Set(normalized.sections.map((section) => section.key));
  const changedResumeSections = normalized.sections.filter((section) => section.source === "resume"
    && JSON.stringify(section.value) !== JSON.stringify(section.originalValue));
  const criterionChecks = {
    required_sections: {
      passed: Boolean(normalized.sections.length) && required.every((section) => section.kind === "list" ? section.value.length > 0 : Boolean(cleanText(section.value))),
      message: "필수 항목이 비어 있지 않아야 합니다.",
    },
    content_depth: { passed: Boolean(normalized.sections.length) && allRequiredComplete, message: "각 항목의 최소 길이와 개수 기준을 충족해야 합니다." },
    placeholder_free: { passed: !placeholder, message: "미확정 자리표시자 문구를 제거해야 합니다." },
    job_focus_coverage: {
      passed: normalized.selection.usedDefaultFocus === true || (focusKeys.size > 0 && [...focusKeys].every((key) => presentKeys.has(key))),
      message: "공고에서 선택한 중점 항목을 문서에 포함해야 합니다.",
    },
    evidence_traceability: {
      passed: changedResumeSections.every((section) => section.sourceRefs.length > 0),
      message: "원문에서 바꾼 이력서 내용에는 승인된 근거를 연결해야 합니다.",
    },
  };
  const criterionDecisions = normalizedCriteria ? normalizedCriteria.filter((item) => item.enabled).map((item) => ({
    key: `criterion:${item.id}`,
    label: item.label,
    weight: item.weight,
    passed: criterionChecks[item.id].passed,
    message: criterionChecks[item.id].passed ? `${item.label} 기준을 충족했습니다.` : criterionChecks[item.id].message,
  })) : [];
  const configuredScore = normalizedCriteria
    ? Math.round(criterionDecisions.reduce((total, item) => total + (item.passed ? item.weight : 0), 0))
    : score;
  const requiredSectionsPresent = required.every((section) => section.kind === "list" ? section.value.length > 0 : Boolean(cleanText(section.value)));
  const hardGatePassed = resumeBodySections.length && !placeholder && (normalizedCriteria ? requiredSectionsPresent : allRequiredComplete);
  const status = hardGatePassed && configuredScore >= threshold ? "passed" : "review";
  const decisions = checks.map(([key, passed, message]) => ({
    key,
    passed,
    message: passed ? "입력 길이와 필수 조건을 충족했습니다." : message,
  }));
  if (!normalized.sections.length) decisions.push({ key: "sections", passed: false, message: "맞춤 수정 항목이 없습니다." });
  if (!resumeBodySections.length) decisions.push({ key: "resume_sections", passed: false, message: "이력서 본문 항목이 없습니다." });
  decisions.push({ key: "placeholder", passed: !placeholder, message: placeholder ? "미확정 자리표시자가 남아 있습니다." : "미확정 자리표시자가 없습니다." });
  if (normalizedCriteria) {
    for (const item of criterionDecisions.filter((decision) => !decision.passed)) {
      if (!findings.some((finding) => finding.key === item.key)) findings.push({ key: item.key, message: item.message });
    }
    decisions.push(...criterionDecisions);
  }
  return { status, score: configuredScore, threshold, findings, decisions, criteria: criterionDecisions };
}
...
function rowToPackage(row, freshness = null) {
  ...
  const content = normalizePackageContent(safeJson(row.content_json, {}));
  const decisions = evaluatePackageQuality(content, {
    threshold: Number(safeJson(row.quality_rules_json, {})?.minimumScore ?? 80),
    criteria: safeJson(row.quality_rules_json, {})?.criteria,
  });
  return {
    id: row.id,
    jobId: row.job_id,
    ...
    diff: content.sections.map((section) => ({
      key: section.key,
      label: section.label,
      changed: JSON.stringify(section.originalValue) !== JSON.stringify(section.value),
      before: section.originalValue,
      after: section.value,
    })),
    sourceFingerprints: {
...
function getPackageRow(db, packageId) {
  return db.prepare(`
    ...
    LEFT JOIN package_review_states r ON r.package_id = p.id
    WHERE p.id = ?
  `).get(packageId);
}
...
export function getLatestPackageForJob(db, jobId, options = {}) {
  ...
  const row = db.prepare(`SELECT p.*, r.status AS review_status, r.note AS review_note
    FROM application_packages p LEFT JOIN package_review_states r ON r.package_id = p.id
    WHERE p.job_id = ? ORDER BY p.version DESC LIMIT 1`).get(jobId);
  if (!row) return null;
...
function protectedFactsFromResume(resume) {
  ...
  const career = resume.career_type === "experienced"
    ? `${resume.career_stage === "career_change" ? "직무전환" : resume.career_stage === "returning" ? "경력복귀" : "경력"}${resume.years_experience === null || resume.years_experience === undefined ? "" : ` · ${resume.years_experience}년`}`
    : "신입";
  const structuredKinds = new Set((Array.isArray(resume.structured_items) ? resume.structured_items : [])
    .filter((item) => item?.active !== false).map((item) => item.kind));
  const standard = [
    ...
    { key: "school", label: "학교", value: resume.school, omit: structuredKinds.has("education") },
    { key: "major", label: "전공", value: resume.major, omit: structuredKinds.has("education") },
    { key: "certificates", label: "자격·인증", value: safeJson(resume.certificates_json, []).join(", "), omit: structuredKinds.has("certification") },
  ].filter((fact) => !fact.omit && cleanText(fact.value)).map(({ omit: _omit, ...fact }) => fact);
  const existing = new Set(standard.map((item) => item.key));
  const confirmed = (Array.isArray(resume.confirmed_facts) ? resume.confirmed_facts : [])
    .filter((fact) => fact?.protected !== false && cleanText(fact?.value) && !existing.has(cleanText(fact?.key, 120)))
    .slice(0, 24)
    .map((fact) => ({ key: cleanText(fact.key, 120), label: cleanText(fact.label, 200), value: cleanText(fact.value, 2000) }));
  return [...standard, ...confirmed];
}
...
export function buildPackageContent(resume = {}, tailoring = {}) {
  ...
  const customSections = (Array.isArray(resume.custom_sections) ? resume.custom_sections : []).filter((item) => item?.key?.startsWith("custom:"));
  for (const item of customSections) if (item.editable !== false) allowed.add(item.key);
  const customByKey = new Map(customSections.map((item) => [item.key, item]));
  const knownKeys = new Set([...Object.keys(SECTION_CATALOG), ...customByKey.keys()]);
  const requested = safeJson(tailoring.focus_sections_json, []).filter((key) => knownKeys.has(key));
  const usedDefaultFocus = requested.length === 0;
  const focus = usedDefaultFocus ? [...Object.keys(SECTION_CATALOG), ...customByKey.keys()] : requested;
  const sections = [];
  for (const key of focus) {
    const custom = customByKey.get(key);
    const definition = SECTION_CATALOG[key] || (custom ? {
      label: custom.label,
      kind: custom.kind,
      maximum: 8000,
      minimum: custom.kind === "list" ? 0 : 20,
      minimumItems: custom.kind === "list" ? 1 : 0,
      minimumItemLength: custom.kind === "list" ? 2 : 0,
    } : null);
    if (!definition || !allowed.has(key)) continue;
    const value = custom ? custom.value : valueFromResume(resume, definition);
  ...
  return normalizePackageContent({
    ...
    structuredItems: resume.structured_items,
...
function currentPackageInputs(db, jobId, options = {}, storedRules = null) {
  ...
  resume.custom_sections = db.prepare(`
    SELECT id, section_key, label, kind, value_json, display_order, editable, source_refs_json
  ...
  resume.confirmed_facts = db.prepare(`
    SELECT fact_key AS key, label, value, protected FROM profile_facts ORDER BY created_at, id
  `).all().map((item) => ({ ...item, protected: Boolean(item.protected) }));
  resume.structured_items = db.prepare(`
    SELECT id, kind, title, organization, role, engagement_type AS engagementType, location, start_date AS startDate,
           end_date AS endDate, summary, highlights_json, skills_json, portfolio_links_json, display_order AS displayOrder, active
  ...
  `).all().map((item) => ({
    ...
    portfolioLinks: safeJson(item.portfolio_links_json, []),
    active: Boolean(item.active),
  }));
  resume.contacts = normalizeContactItems(options.contact);
...
export function createPackage(db, jobId, options = {}) {
  ...
  try {
    ...
    const baseContent = buildPackageContent(inputs.resume, inputs.tailoring);
    const generated = new Map((Array.isArray(options.generatedSections) ? options.generatedSections : [])
      .map((section) => [String(section?.key || ""), section]));
    const content = normalizePackageContent({
      ...baseContent,
      sections: baseContent.sections.map((section) => generated.has(section.key)
        ? { ...section, value: generated.get(section.key).value, sourceRefs: generated.get(section.key).sourceRefs }
        : section),
    });
    const checksum = contentChecksum(content);
    const threshold = Number(inputs.rules.minimumScore ?? 80);
    const quality = evaluatePackageQuality(content, { threshold, criteria: inputs.rules.criteria });
    ...
    if (typeof options.afterApply === "function") {
      const createdRow = db.prepare("SELECT * FROM application_packages WHERE job_id = ? AND version = ?").get(jobId, version);
      options.afterApply(rowToPackage(createdRow, { required: false, reasons: [], available: true }));
    }
    db.exec("COMMIT");
    transactionStarted = false;
  } catch (error) {
...
export function updatePackage(db, packageId, input = {}, options = {}) {
  ...
  try {
    ...
    const updates = new Map((Array.isArray(input.sections) ? input.sections : []).map((section) => [String(section?.key || ""), section]));
    const next = normalizePackageContent({
      ...current,
      sections: current.sections.map((section) => updates.has(section.key) ? {
        ...section,
        value: updates.get(section.key)?.value,
        sourceRefs: options.generatedResult === true ? updates.get(section.key)?.sourceRefs : section.sourceRefs,
      } : section),
    });
    const nextChecksum = contentChecksum(next);
    if (nextChecksum === row.content_checksum) {
      if (typeof options.afterApply === "function") options.afterApply(rowToPackage(row, freshness));
      db.exec("COMMIT");
      transactionStarted = false;
      return rowToPackage(row, freshness);
    }
    const threshold = Number(freshness.inputs.rules.minimumScore ?? 80);
    const quality = evaluatePackageQuality(next, { threshold, criteria: freshness.inputs.rules.criteria });
    ...
    db.prepare("DELETE FROM package_review_states WHERE package_id = ?").run(packageId);
    if (typeof options.afterApply === "function") {
      const currentRow = db.prepare("SELECT * FROM application_packages WHERE id = ?").get(packageId);
      options.afterApply(rowToPackage(currentRow, { required: false, reasons: [], available: true }));
    }
    db.exec("COMMIT");
    transactionStarted = false;
  } catch (error) {
...
export async function approvePackage(db, packageId, options = {}) {
  ...
  if (row.review_status && row.review_status !== "active") {
    throw Object.assign(new Error("Resolve the package review hold or revision request before approval"), { statusCode: 409 });
  }
...
export function prepareSubmission(db, packageId, options = {}) {
  ...
  try {
    ...
    assertLatestMutableVersion(db, row);
    if (row.review_status && row.review_status !== "active") {
      throw Object.assign(new Error("Resolve the package review hold or revision request before submission preparation"), { statusCode: 409 });
    }
    const freshness = packageFreshness(db, row, options);
    if (freshness.required) {
...
export function transitionPackageReview(db, packageId, input = {}, options = {}) {
  ensurePackageIntegrityColumns(db);
  const status = cleanText(input.status, 40);
  if (!new Set(["active", "revision_requested", "on_hold", "cancel_approval"]).has(status)) {
    throw Object.assign(new Error("Unsupported package review transition"), { statusCode: 400 });
  }
  ...
  assertPackageEligible(row);
  assertLatestMutableVersion(db, row);
  if (row.submission_status || new Set(["submit_ready", "submitted"]).has(row.state)) {
    throw Object.assign(new Error("Cancel submission preparation before changing package review state"), { statusCode: 409 });
  }
  let obsoletePdf = "";
  db.exec("BEGIN IMMEDIATE");
  try {
    const current = getPackageRow(db, packageId);
    if (!current || current.state !== row.state || current.content_checksum !== row.content_checksum) {
      throw Object.assign(new Error("The package changed before the review transition"), { statusCode: 409 });
    }
    if (status === "cancel_approval" && current.state !== "approved") {
      throw Object.assign(new Error("Only an approved package can have approval cancelled"), { statusCode: 409 });
    }
    if ((status === "revision_requested" || status === "cancel_approval") && current.state === "approved") {
      obsoletePdf = current.resume_pdf_path || "";
      db.prepare(`INSERT INTO package_approvals (package_id, action, package_checksum, actor_id, note)
        VALUES (?, 'invalidated', ?, 'local-user', ?)`).run(packageId, current.approved_checksum || current.content_checksum, note || "Approval cancelled by user review");
      const nextState = current.quality_status === "passed" ? "approval_pending" : "quality_hold";
      db.prepare(`UPDATE application_packages SET state = ?, resume_pdf_path = '', resume_pdf_checksum = '',
        resume_pdf_pages = 0, approved_checksum = '', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND state = 'approved'`)
        .run(nextState, packageId);
    }
    if (status === "active" || status === "cancel_approval") {
      db.prepare("DELETE FROM package_review_states WHERE package_id = ?").run(packageId);
    } else {
      db.prepare(`INSERT INTO package_review_states (package_id, status, note) VALUES (?, ?, ?)
        ON CONFLICT(package_id) DO UPDATE SET status = excluded.status, note = excluded.note, updated_at = CURRENT_TIMESTAMP`)
        .run(packageId, status, note);
    }
  ...
  if (obsoletePdf) try { fs.rmSync(assertStoredPath(obsoletePdf, "obsolete approved PDF"), { force: true }); } catch {}
  const updated = getPackageRow(db, packageId);
  return rowToPackage(updated, packageFreshness(db, updated, options));
}

export function cancelSubmissionPreparation(db, packageId, options = {}) {
  ensurePackageIntegrityColumns(db);
  const row = getPackageRow(db, packageId);
  if (!row) throw Object.assign(new Error("Package not found"), { statusCode: 404 });
  assertPackageEligible(row);
  assertLatestMutableVersion(db, row);
  if (row.state !== "submit_ready" || row.submission_status !== "submit_ready") {
    throw Object.assign(new Error("Only a prepared, unsubmitted package can be cancelled"), { statusCode: 409 });
  }
  verifiedPdf(row.frozen_pdf_path, row.frozen_pdf_checksum, row.frozen_pdf_pages);
  db.exec("BEGIN IMMEDIATE");
  try {
    const deleted = db.prepare("DELETE FROM package_submissions WHERE package_id = ? AND status = 'submit_ready'").run(packageId);
    const restored = db.prepare(`UPDATE application_packages SET state = 'approved', updated_at = CURRENT_TIMESTAMP
      WHERE id = ? AND state = 'submit_ready' AND approved_checksum = content_checksum`).run(packageId);
    if (deleted.changes !== 1 || restored.changes !== 1) throw Object.assign(new Error("Submission preparation changed before cancellation"), { statusCode: 409 });
    if (typeof options.beforeCommit === "function") options.beforeCommit();
    db.exec("COMMIT");
  } catch (error) {
    try { db.exec("ROLLBACK"); } catch {}
  ...
  try {
    const frozen = assertStoredPath(row.frozen_pdf_path, "frozen submission PDF");
    if (frozen !== row.resume_pdf_path) fs.rmSync(frozen, { force: true });
    try { fs.rmdirSync(path.dirname(frozen)); } catch {}
  } catch {}
  const updated = getPackageRow(db, packageId);
  return rowToPackage(updated, packageFreshness(db, updated, options));
}
...
export function publicPackage(value) {
  ...
  return {
    ...
    reviewStatus: value.reviewStatus || "active",
