...
export function renderPackageMarkdown(content, job = {}) {
  ...
  for (const kind of Object.keys(structuredLabels)) {
    ...
    for (const item of items) {
      const metadata = [item.organization, item.role, [item.startDate, item.endDate].filter(Boolean).join(" ~ "), item.location].filter(Boolean).join(" · ");
...
export function renderPackageHtml(content, job = {}) {
  ...
  const structured = Object.entries(structuredLabels).map(([kind, label]) => {
    ...
    return `<section><h2>${escapeHtml(label)}</h2>${items.map((item) => {
      const metadata = [item.organization, item.role, [item.startDate, item.endDate].filter(Boolean).join(" ~ "), item.location].filter(Boolean).join(" · ");
      return `<article class="structured-item"><h3>${escapeHtml(item.title)}</h3>${metadata ? `<div class="meta">${escapeHtml(metadata)}</div>` : ""}${prose(item.summary)}${item.highlights.length ? `<ul>${list(item.highlights)}</ul>` : ""}${item.skills.length ? `<div class="skills">${escapeHtml(item.skills.join(" · "))}</div>` : ""}</article>`;
    }).join("")}</section>`;
  }).join("");
  return `<!doctype html>
...
function currentPackageInputs(db, jobId, options = {}, storedRules = null) {
  ...
  resume.structured_items = db.prepare(`
    SELECT id, kind, title, organization, role, location, start_date AS startDate,
           end_date AS endDate, summary, highlights_json, skills_json, display_order AS displayOrder, active
