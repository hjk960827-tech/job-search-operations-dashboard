...
import { CONFIG_DIR, PRIVATE_DATA_DIR, assertPathInside, configPath, onboardingDocumentPath } from "./paths.mjs";
...
function privateDocumentRow(db, documentId) {
  const row = db.prepare(`
    SELECT id, kind, original_name, internal_path, mime_type, sha256, active
    FROM source_documents WHERE id = ?
  `).get(text(documentId, 120));
  if (!row) throw settingsError("등록 문서를 찾을 수 없습니다.", 404);
  const filePath = assertPathInside(PRIVATE_DATA_DIR, row.internal_path, "private document path");
  if (!fs.existsSync(filePath)) throw settingsError("등록 문서 원본을 찾을 수 없습니다.", 404);
  const stat = fs.lstatSync(filePath);
  if (stat.isSymbolicLink() || !stat.isFile()) throw settingsError("등록 문서 경로가 안전하지 않습니다.", 409);
  return { ...row, filePath };
}

export function readPersonalDocument(db, documentId) {
  const row = privateDocumentRow(db, documentId);
  const body = fs.readFileSync(row.filePath);
  const checksum = crypto.createHash("sha256").update(body).digest("hex");
  if (row.sha256 && checksum !== row.sha256) throw settingsError("등록 문서 체크섬이 일치하지 않습니다.", 409);
  return { body, fileName: row.original_name || "document", mimeType: row.mime_type || "application/octet-stream" };
}

export function deletePersonalDocument(db, documentId) {
  const row = privateDocumentRow(db, documentId);
  const eventKey = crypto.randomUUID();
  const quarantine = `${row.filePath}.purge-${eventKey}`;
  fs.renameSync(row.filePath, quarantine);
  let committed = false;
  try {
    inTransaction(db, () => {
      const removed = db.prepare("DELETE FROM source_documents WHERE id = ?").run(row.id);
      if (Number(removed.changes) !== 1) throw settingsError("삭제할 문서가 변경되었습니다.", 409);
      db.prepare(`INSERT INTO privacy_deletion_events
        (event_key, document_id, document_kind, document_sha256, status)
        VALUES (?, ?, ?, ?, 'deleted')`).run(eventKey, row.id, row.kind, row.sha256 || "");
    });
    committed = true;
    let deletionStatus = "deleted";
    try {
    ...
    try { fs.rmdirSync(path.dirname(row.filePath)); } catch {}
    return { documents: publicDocuments(db), deletion: { eventKey, status: deletionStatus } };
  } catch (error) {
    if (!committed && fs.existsSync(quarantine) && !fs.existsSync(row.filePath)) fs.renameSync(quarantine, row.filePath);
    throw error;
  }
}
