...
const ENUMS = {
  ...
  score: new Set(["", "70", "80", "90", "unset"]),
  condition: new Set(["", "remote", "fulltime", "contract", "entry", "experienced"]),
  sort: new Set(["score", "deadline", "recent", "company"]),
};
...
export function normalizeJobFilters(input = {}) {
  ...
  const unknown = Object.keys(input).filter((key) => !new Set(["search", "track", "region", "score", "platform", "status", "lifecycle", "deadline", "condition", "sort", "favorite"]).has(key));
  ...
  const result = {
    ...
    region: cleanText(input.region, "filters.region", 160),
    score: cleanText(input.score, "filters.score", 20),
    ...
    condition: cleanText(input.condition, "filters.condition", 30),
