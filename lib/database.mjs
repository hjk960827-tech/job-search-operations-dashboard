...
export function updateApplicationState(db, jobId, patch) {
  ...
  `).get(jobId);
  if (!current) throw inputError("Job not found", 404);
  ...
  if (!allowedStatuses.has(workflowStatus)) throw inputError("Unsupported workflow status");
...
export function importJob(db, input) {
  ...
  for (const key of required) {
    if (!String(input[key] || "").trim()) throw inputError(`Missing required field: ${key}`);
  }
  const job = {
  ...
  for (const source of Array.isArray(input.sources) ? input.sources.slice(0, 20) : []) {
    ...
    try {
      ...
      throw inputError("Job source URL is not a valid URL");
    }
    if (!new Set(["http:", "https:"]).has(url.protocol)) throw inputError("Only HTTP(S) job sources are allowed");
    addSource.run(
      jobId,
  ...
  if (input.score !== undefined && input.score !== null) {
    ...
    if (!Number.isFinite(score)) throw inputError("Score must be a number between 0 and 100");
    db.prepare(`
