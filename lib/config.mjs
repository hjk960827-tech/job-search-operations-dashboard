...
import { validateDocumentQualityCriteria } from "./document-quality.mjs";
...
export function validateConfig(name, value, { requireComplete = true } = {}) {
  ...
  if (name === "profile") {
    ...
    if (!validTimeZone(input.location?.timezone)) issues.push(issue("profile.location.timezone must be a valid IANA time zone"));
    if (input.identity?.pdf_fields !== undefined) {
      if (!input.identity.pdf_fields || typeof input.identity.pdf_fields !== "object" || Array.isArray(input.identity.pdf_fields)) {
        issues.push(issue("profile.identity.pdf_fields must be an object"));
      } else {
        for (const field of ["email", "phone", "address"]) {
          if (typeof input.identity.pdf_fields[field] !== "boolean") {
            issues.push(issue(`profile.identity.pdf_fields.${field} must be true or false`));
          }
          if (input.identity.pdf_fields[field] === true && !String(input.identity?.[field] || "").trim()) {
            issues.push(issue(`profile.identity.${field} is required when included in PDF`));
          }
        }
      }
    }
    ...
    issues.push(...validateDocumentQualityCriteria(input.quality_rules?.criteria));
