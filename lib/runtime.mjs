...
export function isAllowedRequestHost(value, port) {
  return parseLocalAuthority(value, port);
}

export function isAllowedRequestOrigin(value, port) {
...
export function protectLocalRequest({ method, pathname, headers, port, mode }) {
  if (!isAllowedRequestHost(headerValue(headers, "host"), port)) {
    throw new HttpRequestError(403, "허용되지 않은 Host 요청입니다.");
  }

  const origin = headerValue(headers, "origin");
  if (origin !== undefined && !isAllowedRequestOrigin(origin, port)) {
  ...
  if (!isApplicationJson(headerValue(headers, "content-type"))) {
