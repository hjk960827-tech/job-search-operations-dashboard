...
export function normalizeStructuredItems(input, db = null) {
  ...
  return input.map((item, index) => {
    ...
    const normalized = {
      ...
      engagementType: text(item.engagementType, `structuredItems[${index}].engagementType`, 200),
      ...
      portfolioLinks: textList(item.portfolioLinks, `structuredItems[${index}].portfolioLinks`, 30, 1000),
...
export function replaceStructuredResumeItems(db, items) {
  ...
  const insert = db.prepare(`
    INSERT INTO resume_structured_items (
      id, kind, title, organization, role, engagement_type, location, start_date, end_date, summary,
      highlights_json, skills_json, portfolio_links_json, source_refs_json, display_order, active
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  for (const item of items) {
    insert.run(item.id, item.kind, item.title, item.organization, item.role, item.engagementType, item.location,
      item.startDate, item.endDate, item.summary, JSON.stringify(item.highlights), JSON.stringify(item.skills),
      JSON.stringify(item.portfolioLinks), JSON.stringify(item.sourceRefs), item.displayOrder, item.active ? 1 : 0);
  }
}
