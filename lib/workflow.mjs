...
export function deriveJobWorkflow(job) {
  ...
  if (packageValue?.reviewStatus === "revision_requested") {
    return workflowValue(job, "quality", { label: "보완 요청", description: packageValue.reviewNote || "사용자가 문안 보완을 요청했습니다." });
  }
  if (packageValue?.reviewStatus === "on_hold") {
    return workflowValue(job, "quality", { label: "검토 보류", description: packageValue.reviewNote || "사용자가 이 작업본의 판단을 보류했습니다.", nextAction: null });
  }
...
export function deriveJobAllowedActions(job) {
  ...
  const reviewBlocked = Boolean(packageValue && packageValue.reviewStatus && packageValue.reviewStatus !== "active");
  ...
  return {
    ...
    approvePackage: action(
      Boolean(editablePackage && packageValue?.state === "approval_pending" && !reviewBlocked),
      reviewBlocked ? "보완 요청 또는 보류 상태를 해제한 뒤 승인할 수 있습니다." : "품질 기준을 통과한 승인 대기 문서만 승인할 수 있습니다.",
    ),
    prepareSubmission: action(
      submissionReady && !reviewBlocked,
      reviewBlocked ? "보완 요청 또는 보류 상태를 해제한 뒤 제출을 준비할 수 있습니다." : "승인된 최신 문안과 검증된 PDF가 모두 준비되어야 합니다.",
    ),
