...
예시 데이터로 시작합니다. 현재 버전은 `0.1.1`입니다.
...
| `config/resume.yml` | 문서 최소 작성 기준과 PDF 최대 페이지 수 (`document.example.yml`에서 생성) |
...
공고 적합도 `score`는 외부 채점기가 전달한 0~100 숫자를 표시하는 확장 지점입니다.
현재 내장 채점기는 없으며 `search.yml`의 `scoring.review_below`는 낮은 점수 경고에만
사용됩니다. 세부 점수 분석 UI는 후속 범위입니다.
...
현재 PDF는 첨부 이력서를 자동 해석하지 않고 대시보드에 입력한 9개 직무 중립
항목(헤드라인, 요약, 역량, 경력 하이라이트, 성과 근거, 대표 경험, 직접 담당,
협업 범위, 일하는 방식)에서 만든 공고별 작업본입니다. 설치당 활성 기본 이력서는 1개입니다.
`profile.yml`의 이메일·전화번호와 주소는 아직 PDF에 반영되지 않으므로
연락처와 자유 섹션까지 갖춘 최종 제출 이력서로 간주하면 안 됩니다. 사용자는 제출 전
내용과 연락처·레이아웃을 별도로 확인해야 합니다. PDF 산출물은 내부 안전 파일명을 사용하며
사용자 파일명 규칙은 제공하지 않습니다. 임의 커스텀 섹션과 외부 HTML 입력은
후속 확장 범위입니다.
